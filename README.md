...
単純に最も近いピクセルの色を使用するため、縮小時にジャギー（ギザギザ）が目立つ。
処理速度が最優先の場合やピクセルアートなど意図的に鮮明なエッジを維持したい場合に向いている。

- **Bilinear**

高速だが、画質が粗い。

周囲の 4 ピクセルを使用した線形補間で、最近傍法よりは滑らかだがぼやける傾向がある。
高速なプレビューや低解像度画像の処理に向いている。

- **Bicubic**

`Bilinear` より画質が良くなるが、速度が遅くなる。

周囲の 16 ピクセルを使用した 3 次補間で、エッジの保存性が向上する。
一般的な写真のリサイズで十分な品質と速度のバランスが得られる。

- **Mitchell Netravali**

`Bicubic` より画質が良くなるが、速度が遅くなる。

バイキュービックの一種だが、リンギング（波状のアーティファクト）を抑制する特性を持つ。
高品質な写真リサイズでエッジをシャープに保ちたい場合に向いている。

- **Lanczos2**

`Mitchell Netravali` 画質がより良くなるが、速度が遅くなる。

2 タップのシンク関数を使用し、シャープな詳細を保持しながらも高品質のリサイズを実現する。
高品質な画像縮小で処理時間をやや抑えたい場合に向いている。

- **Lanczos3**

これまでの中で最も画質が良くなるが、速度も最も遅くなる。

3 タップのシンク関数を使用し、最もシャープで詳細を保持する高品質アルゴリズム。
最終出力品質が最優先の場合や、印刷用など高品質な画像処理が必要な場合に向いている。

- 処理速度の比較

NearestNeighbor (最速) > Bilinear > Bicubic > Mitchell-Netravali > Lanczos2 > Lanczos3 (最遅)

- 画質の比較

NearestNeighbor (最低) < Bilinear < Bicubic < Mitchell-Netravali < Lanczos2 < Lanczos3 (最高)
